    ...
    for c = group.first,group.last do
        ...
        used_size = math.max(used_size, geom[width] + cls[c].border_width * 2)
         ...
         then
             ...
             if top then
                 geom[height] = geom[height] - 2 * beautiful.useless_gap
                 geom[y] = geom[y] + beautiful.useless_gap
             else
                 geom[height] = geom[height] - beautiful.useless_gap
             end
    
             if left then
                 geom[width] = geom[width] - 2 * beautiful.useless_gap
                 geom[x] = geom[x] + beautiful.useless_gap
             else
                 geom[width] = geom[width] - beautiful.useless_gap
             end
         end
         -- End of useless gap.
